...
The guest python scripts (`image/*.py`) have pytest units here in `offline/`
(`test_*.py`). pre-commit runs them with a pinned pytest in a venv it manages,
so the runner is identical locally and in CI and nothing is assumed installed.
